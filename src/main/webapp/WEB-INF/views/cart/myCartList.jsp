...
<c:set var="totalDiscountedPrice" value="0"/><!-- 총 할인후 금액 -->
<c:set  var="totalDiscountPrice" value="0" /> <!-- 총 할인금액 -->
<head>
...
<c:forEach var="item" items="${myGoodsList}" varStatus="cnt">
		...
		<!-- 할인가  --> 
				...
				<c:set var="discounted_price" value="${item.goods_sales_price*left}"/>
		<!-- 할인 -->		
				<c:set var="discount_price" value="${item.goods_sales_price-discounted_price}"/>
				
				
					<td><input type="checkbox" name="checked_goods"  checked  value="${item.goods_id}"  onClick="calcGoodsPrice(${discounted_price},this)"></td>
		...
		<!-- 정가-->	<td class="price"><span>${goodsprice}원</span></td>
					
		<!-- 판매가 -->	
				<fmt:formatNumber value="${item.goods_sales_price}" var="salesprice" pattern="#,###"/>	
					<td class="price"><span>${salesprice}</span>
						
				<fmt:formatNumber value="${discounted_price}" var="discountedprice" pattern="#,###"/>
						<strong>
				            ${discountedprice}원(${discount}% 추가할인)
				         </strong>
					</td>
					<td>
					   <input type="text" id="cart_goods_qty" name="cart_goods_qty" size=3 value="${cart_goods_qty}"><br>
						<a href="javascript:modify_cart_qty(${item.goods_id},${discounted_price},${cnt.count-1 });" >
						    <img width=25 alt=""  src="${contextPath}/resources/image/btn_modify_qty.jpg">
						</a>
					</td>
					<td>
					   <strong>
		<!-- 합계 -->			    <fmt:formatNumber  value="${discounted_price*cart_goods_qty}" type="number" var="total_sales_price" pattern="#,###"/>
				         ${total_sales_price}원
					</strong> </td>
					<td>
		<!-- 각각구매 -->	 <a href="javascript:fn_order_each_goods('${item.goods_id }','${item.goods_title }','${discounted_price}','${item.goods_fileName}');">
...
<!--총 할인금액 -->  
          <td>  
            ...
            <fmt:formatNumber value="${totalDiscountPrice}" var="tDiscountPrice" pattern="#,###"/>
                     ${tDiscountPrice}원
            </p>
            <input id="h_totalSalesPrice"type="hidden" value="${totalDiscountPrice}" />
          </td>
          <td>  
...
<!-- 할인 후 최종금액-->
  		<!--총판매가+배송비 - 총할인금액 -->
             ...
             <fmt:formatNumber  value="${totalGoodsPrice+totalDeliveryPrice-totalDiscountPrice}" type="number" var="total_price" pattern="#,###"/>
               ${total_price}원
         <!--  할인 후 금액 + 배송비
