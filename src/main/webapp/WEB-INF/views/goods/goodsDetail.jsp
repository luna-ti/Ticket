...
<body>
	<c:if test="${goodsMap.goodsVO.goods_sort eq 'it'}">
		<c:set var="sort" value="IT/인터넷"/>
	</c:if>
	<c:if test="${goodsMap.goodsVO.goods_sort eq 'human'}">
		<c:set var="sort" value="역사/인문학"/>
	</c:if>
	<c:if test="${goodsMap.goodsVO.goods_sort eq 'science'}">
		<c:set var="sort" value="자연과학/공학"/>
	</c:if>
	<c:if test="${goodsMap.goodsVO.goods_sort eq 'improve'}">
		<c:set var="sort" value="자기계발"/>
	</c:if>
	<c:if test="${goodsMap.goodsVO.goods_sort eq 'college'}">
		<c:set var="sort"  value="대학교재"/>
	</c:if>
	<c:if test="${goodsMap.goodsVO.goods_sort eq 'economy'}">
		<c:set var="sort" value="경제/경영"/>
	</c:if>
	<c:choose>
		<c:when test="${goodsMap.goodsVO.goods_status eq 'bestseller'}">
			<c:set var="status" value="베스트셀러"/>
		</c:when>
		<c:when test="${goodsMap.goodsVO.goods_status eq 'steadyseller'}">
			<c:set var="status" value="스테디셀러"/>
		</c:when>
		<c:when test="${goodsMap.goodsVO.goods_status eq 'newbook'}">
			<c:set var="status" value="신간"/>
		</c:when>
		<c:when test="${goodsMap.goodsVO.goods_status eq 'on_sale'}">
			<c:set var="status" value="판매중"/>
		</c:when>
		<c:when test="${goodsMap.goodsVO.goods_status eq 'buy_out'}">
			<c:set var="status" value="품절"/>
		</c:when>
		<c:otherwise>
			<c:set var="status" value="절판"/>
		</c:otherwise>
	</c:choose>
	<hgroup>
		<h1>${sort}</h1>
		<h2>국내외 도서 &gt; ${sort} &gt;${status}</h2>
